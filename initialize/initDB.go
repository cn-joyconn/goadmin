...
func _initAdminUser() {
	...
	for i, userObj := range global.Admins.Users {
		if userObj.ID < 1 {
			...
			adminUser.ID = adminModel.Juint64(snowflake.NextId())
			adminUser.Password = adminService.GetSaltPwd(adminUser.ID, "admin123")
			insertResult, adminUser := adminService.InsertUserModel(adminUser)
			if insertResult > 0 {
				adminUsers = append(adminUsers, uint64(adminUser.ID))
				userObj.ID = uint64(adminUser.ID)
